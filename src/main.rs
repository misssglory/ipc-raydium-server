...
use ipc_server_rs::{SwapExecutor, SwapResult, handlers};
  ...
  match found_addresses.len() {
    1 => {
      ...
      if hash_cache.insert(address) {
        for i in 0..1 {
          handlers::round_trip(executor.clone(), Some(&address)).await;
          // handlers::test_quote(executor.clone(), Some(&address), i).await;
        }
      }
  ...
  loop {
    ...
    match reader.read_line(&mut buffer).await {
      ...
      Ok(_) => {
        ...
        let request = IpcRequest { message: buffer.clone(), correlation_id: None };
